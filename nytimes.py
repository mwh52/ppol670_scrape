...
def left_over():
    try:
        current = scraperwiki.sqlite.execute(
            "SELECT keyword, page FROM nytimes ORDER BY epoch_time DESC LIMIT 1")
        current_word = current['data'][0][0]
        current_page = current['data'][0][1]
        return [current_word, current_page]
    except:
        scraperwiki.sqlite.execute(
            "CREATE TABLE nytimes (epoch_time int, id string, date_time string, keyword string, page int)")
